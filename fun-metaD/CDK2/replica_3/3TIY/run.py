...
Steps:
    ...
    4. 500ns well-tempered funnel-metadynamics
...
if os.path.isfile('checkpnt.chk'):
    ...
    if os.path.isfile('sim_log.csv'):
        os.rename('sim_log.csv','1_sim_log.csv')
    sim_log_file = [ line[:-2] for line in open('1_sim_log.csv').readlines()]
    current_steps = int(sim_log_file[-1].split(',')[1])
    # the sim log only said 500ns, 
    # so also add another 500 to give it a 1us total
    steps = run_time * 250000 - (current_steps + 500.0 * 250000)
    if os.path.isfile('COLVAR.npy'):
        os.rename('COLVAR.npy','1_COLVAR.npy')
    trj_name = '2_trj.dcd'
    # rename the previous bias file, so not too much bias is read
    ...
    older_bias_file = bias_files_by_age[0]
    os.rename(older_bias_file, 'bias_dir/_' + older_bias_file.split('/')[-1])
else:
...
coords_file = '../../input/3TIY/solvated.rst7'
parm_file = '../../input/3TIY/solvatedHMR.prmtop'
...
lig = [ i for i in range(4848, 4872)]
p1 = [2370, 2382, 2402, 2409, 2428]
p2 = [162, 181, 188, 203, 252, 268, 284, 305, 468, 484, 494, 513, 1021, 1037, 1059, 1266, 1285, 1301, 1321, 1336, 1356, 1375, 1392, 1409, 2152, 2169, 2183, 2202, 2341, 2360, 2370]
...
proj = BiasVariable(projection, 0.30, 4.20, 0.025, False, gridWidth = 200)
...
ext = BiasVariable(extent, 0.0, 1.10, 0.05, False, gridWidth = 200)
...
simulation.reporters.append(StateDataReporter(
                           '2_sim_log.csv', 250000, step=True,
...
for i in range(0,int(steps),500):
    ...
    if i%(250000/2) == 0 and i != 0:
        paths = sorted(Path('bias_dir/').iterdir(), key=os.path.getmtime)
        bias_files_by_age = [ str(fpath) for fpath in paths if 'bias' in str(fpath) ]
